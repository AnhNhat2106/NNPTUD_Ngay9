...
router.post("/import", uploadHandler.uploadExcel.single('file'), async function (req, res, next) {
  try {
    if (!req.file) {
      return res.status(400).send({ message: "No file uploaded. Please upload an Excel (.xlsx) file." });
    }
    
    let result = await userController.ImportUsersExcel(req.file.path);
    res.send(result);
  } catch (err) {
    res.status(400).send({ message: err.message });
  }
